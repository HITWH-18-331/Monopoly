...
while True:
    ...
    if gameManager.gameTurn is GameTurn.initial:
        ...
        for i in range(3, 49, 3):
            if i % 5 != 0:
                lands.lands[i].owner = hero.name
                lands.lands[i].level = i % 4 + 1
                lands.lands[i].incident = Incidents.house
                for j in range(lands.lands[i].level):
                    hero.houseCounter[j] += 1
        for i in range(2, 49, 3):
            if i % 5 != 0:
                lands.lands[i].owner = enemy.name
                lands.lands[i].level = i % 4 + 1
                lands.lands[i].incident = Incidents.house
                for j in range(lands.lands[i].level):
                    enemy.houseCounter[j] += 1
    ...
    if gameManager.gameTurn is GameTurn.playing:
        ...
        if gameManager.playerTurn is PlayerTurn.PCMove:
            ...
            # PC购买地块、交换金钱、使用炸药
            if gameManager.PCActKey[0] is True:
                hero.buy(lands.lands[hero.position])
                if lands.lands[hero.position].incident is Incidents.changeMoney and hero.chance is True:
                    temp = hero.money
                    ...
                    # 测试代码
                    print("ENEMY的第", hero.position, "地块被炸掉了")
                gameManager.PCActKey[0] = False                                           # 每次响应过按键之后重置

                # 测试代码
                print("hero:", hero.houseCounter)
                print("Hero:", end=" ")
                for land in lands.lands:
                    if land.owner == hero.name:
                        print(str(land.position) + "-" + str(land.level), end="  ")
                print("")

            # 千里马额外移动
            if hero.transportation == "千里马":
                if gameManager.PCActKey[1] is True:
                    hero.swift_horse_move(False)
                    gameManager.PCActKey[1] = False                                       # 每次响应过按键之后重置
                elif gameManager.PCActKey[2] is True:
                    hero.swift_horse_move(True)
                    gameManager.PCActKey[2] = False                                       # 每次响应过按键之后重置
            ...
            if lands.lands[enemy.position].incident is Incidents.changeMoney and hero.money > enemy.money:
                ...
                # 测试代码
                print("HERO的第", enemy.position, "地块被炸掉了")
            for i in range(5):
            ...
            # 测试代码
            print("enemy:", enemy.houseCounter)
            print("Enemy:", end=" ")
            for land in lands.lands:
                if land.owner == enemy.name:
                    print(str(land.position) + "-" + str(land.level), end="  ")
            print("\n")
        ...
        # 测试代码
        pygame.event.post(gameManager.spaceKeyDown)

    if gameManager.gameTurn is GameTurn.over:
