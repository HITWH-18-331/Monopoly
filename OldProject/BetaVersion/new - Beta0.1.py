...
# 角色
class character:
    def __init__(self, name, image, whoControl):
        self.playerControl = whoControl
        self.name = name
        self.position = 0   # 初始位置
        self.money = 1000   # 初始资金
        self.image = image   # 外观图片
        self.houseLocation = []    # 房间位置，使用元组的列表(location, level)
        ...
        self.cheat = list()
        self.messageBoardLocation = list(tuple())
        for i in range(3):
            self.cheat.append(1 if self.playerControl is True else 0)
    ...
    def display(self, Ls):
        if self.check() is True:
            self.__displayBase()
        if self.playerControl is True:
            self.__displayIncidents(Ls.lands, Ls.incidents)

    def __displayBase(self):
        # 地图坐标
        printPosition = font.render('坐标: %d' % hero.position, True, [0, 0, 0])
        screen.blit(printPosition, [80, 100])
        # 状态
        printStatus = font.render('状态: %s' % hero.status, True, [0, 0, 0])
        screen.blit(printStatus, [200, 100])
        # 交通工具
        printTransportation = font.render('装备: %s' % hero.transportation, True, [0, 0, 0])
        screen.blit(printTransportation, [80, 124])
        # 物品
        printTransportation = font.render('物品: %s' % hero.item, True, [0, 0, 0])
        screen.blit(printTransportation, [200, 124])
        # 金钱
        printMoney = font.render('资金: %d金币' % hero.money, True, [0, 0, 0])
        screen.blit(printMoney, [80, 148])
    ...
    def __displayIncidents(self, L, incidents):
        ...
        if L[location].owner != "事件":
            ...
            if L[location].owner == "系统":
                printHouse = font.render('这里是一片无主的荒地', True, [0, 0, 255])
                printTip = font.render('按B键花费$%d建立城堡' % ((level + 1) * 100), True, [0, 0, 255])
                if self.free == 1:
                    printTip3 = font.render('你可以免费建立城堡（仅限一次）', True, [0, 0, 255])
                    screen.blit(printTip3, [80, 228])
            elif L[location].owner == self.name:
                printHouse = font.render('城墙上的卫兵向你举旗致敬', True, [0, 0, 255])
                if L[location].level < 5:
                    printTip = font.render('按B键花费$%d升级城堡' % ((level + 1) * 100), True, [0, 0, 255])
                    if self.free == 1:
                        printTip3 = font.render('你可以免费升级城堡（仅限一次）', True, [0, 0, 255])
                        screen.blit(printTip3, [80, 228])
                else:
                    printTip = font.render('城堡已经很豪华了！', True, [0, 0, 255])
            else:
                printHouse = font.render('高耸的城堡阴沉地矗立于前方', True, [0, 0, 255])
                printTip = font.render('你到了别人的地盘，不得不支付$%d过路费' % (level * 100), True, [0, 0, 255])
                # 炸药使用提示
                if self.item == '炸药':
                    printTip3 = font.render('按B键使用炸药炸毁城堡', True, [0, 0, 255])
                    screen.blit(printTip3, [80, 228])
            screen.blit(printHouse, [80, 180])
            screen.blit(printTip, [80, 204])
        # 事件1: 到达马场
        elif L[location].incident == incidents[1]:
            printEvent = font.render('事件：你来到了马场', True, [0, 0, 255])
            screen.blit(printEvent, [80, 180])
            # 走路状态想买马
            if self.transportation == '无':
                printTip1 = font.render('按B键花费$1000购买一匹战马', True, [0, 0, 255])
                printTip2 = font.render('现在你以双倍速度进行移动！', True, [0, 0, 255])
            # 买了马想升级
            elif self.transportation == '战马':
                printTip1 = font.render('按B键花费$2000将战马升级为千里马', True, [0, 0, 255])
                printTip2 = font.render('骑上千里马的你可以进行额外行动！', True, [0, 0, 255])
                printTip3 = font.render('按A键向前一格，按D键向后一格', True, [0, 0, 255])
                screen.blit(printTip3, [80, 252])
            # 战马升级完毕
            elif self.transportation == '千里马':
                printTip1 = font.render('这里已经没有值得购买的好马了', True, [0, 0, 255])
                printTip2 = font.render('你失望地离开了马场', True, [0, 0, 255])
            screen.blit(printTip1, [80, 204])
            screen.blit(printTip2, [80, 228])
        # 事件2：遭遇小偷
        elif L[location].incident == incidents[2]:
            printEvent = font.render('事件：走在大街上的你遇到了小偷', True, [0, 0, 255])
            screen.blit(printEvent, [80, 180])
            if self.transportation == '无':
                printTip = font.render('你失去了$500', True, [0, 0, 255])
            else:
                printTip = font.render('幸运的是，骑在马上的你没有成为小偷的目标', True, [0, 0, 255])
            screen.blit(printTip, [80, 204])
        # 事件3：逮捕入狱
        elif L[location].incident == incidents[3]:
            if self.status == '正常' or self.status == '监禁':
                printEvent = font.render('事件: 你被卷入一场谋杀案，暂时无法脱身', True, [0, 0, 255])
                printTip = font.render('你本回合无法移动', True, [0, 0, 255])
            elif self.status == '感冒':
                printEvent = font.render('事件: 你因生病而在医院修养，渡过了平静的一天', True, [0, 0, 255])
                printTip = font.render('你幸运地避开了谋杀案的牵连', True, [0, 0, 255])
            screen.blit(printEvent, [80, 180])
            screen.blit(printTip, [80, 204])
        # 事件4：资金互换
        elif L[location].incident == incidents[4]:
            printEvent1 = font.render('事件: 你碰巧获得了一个与他人互换财富的机会', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('按B键与敌人互换资金', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])
        # 事件5：获得炸药
        elif L[location].incident == incidents[5]:
            printEvent1 = font.render('事件: 你在路边捡到了炼金术士丢弃的炸药', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('在敌人的城堡中按B键使用炸药，仅限一次', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])
        # 事件6：资产暴增
        elif L[location].incident == incidents[6]:
            printEvent1 = font.render('事件: 你在山间偶然发现了一座金矿', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('你得到了$1000', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])
        # 事件7：生病
        elif L[location].incident == incidents[7]:
            printEvent1 = font.render('事件: 你发现自己得了感冒，还好病得不算严重', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('你在三回合内的移动速度减半', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])
        # 事件8：遭遇大风
        elif L[location].incident == incidents[8]:
            printEvent1 = font.render('事件:你被一阵狂风卷起，身不由己地飞了起来', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('你下次移动后将在随机位置出现', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])
        # 事件9：领主结好
        elif L[location].incident == incidents[9]:
            printEvent1 = font.render('事件:你与本地领主建立友谊，得到了他的承诺', True, [0, 0, 255])
            screen.blit(printEvent1, [80, 180])
            printTip2 = font.render('你下次修建或升级城堡完全免费', True, [0, 0, 255])
            screen.blit(printTip2, [80, 204])

    def shopping(self, Ls):
        if self.playerControl is True:
            return self.__buy(Ls.lands, Ls.cattleOfPC)
        else:
            return self.__autoBuy(Ls.lands, Ls.cattleOfNPC)

    def __buy(self, L, cattleOfPC):
        # 购买地块
        location = self.position
    ...
    def __autoBuy(self, L, cattleOfNPC):
        # 购买地块
        location = self.position
        if self.money > 500:
            price = L[location].price(self.name)
    ...
    def incidents(self, Ls):
        ...
        location = self.position
        # 1.路过敌人房子
        if L[location].owner != "系统" and L[location].owner != self.name and L[location].owner != "事件":
            self.money -= L[location].level * 100 - 100
        # 特殊事件房间
...
class landmasses:
    def __init__(self, PCName, NPCName):
        ...
        self.cattleOfPC.append(wasteland)
        ...
        self.cattleOfNPC.append(wasteland)
        ...
        class oneLand:
            ...
            def changeProperty(self, who, pcName = self.PCName,
...
wasteland = pygame.image.load("./source/wasteland.png")
...
# 游戏角色
hero = character("PC", PCImage, True)
enemy = character("NPC", NPCImage, False)
...
while True:
    ...
    for land in lands.lands:
        screen.blit(land.image, locationChange(land.position))
    # 人物绘制
    screen.blit(enemy.image, locationChange(enemy.position))
    screen.blit(hero.image, locationChange(hero.position))
    # 信息显示
    hero.display(lands)
    enemy.display(lands)
    # 失败判定
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if heroTurn is True:
                if event.key == K_SPACE:
                    hero.move()
                    hero.incidents(lands)
                    heroTurn, enemyTurn = False, True
                    hero.engine = 0
            # 购买地块、交换金钱、使用炸药
            if event.key == K_b:
                if lands.lands[hero.position].owner != "事件" and lands.lands[hero.position].owner != enemy.name:
                    hero.shopping(lands)
                ...
                # 在下方的分支解决使用炸药后wasteland的赋值问题


                elif lands.lands[hero.position].owner == enemy.name and hero.item == '炸药':
                    lands.lands[hero.position].owner = "系统"
                    lands.lands[hero.position].level = 0
                    lands.lands[hero.position].image = wasteland
                    lands.lands[hero.position].incident = "无"
                    hero.item = '无'
            # 引擎移动
    ...
    if enemyTurn is True:
        enemy.move()
        enemy.incidents(lands)
        enemy.shopping(lands)
        heroTurn, enemyTurn = True, False
