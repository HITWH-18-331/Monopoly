...
class NPC:
    ...
    def swift_horse_move(self, lands, land_is_full):
        if self.money > 900 and self.transportation == "千里马":
            ...
            for i in range(5):
                self.buy(lands[forward], land_is_full)
            for i in range(5):
                self.buy(lands[backward], land_is_full)
...
class GameManager:
    def __init__(self):
        ...
        self.PCActKey = [pygame.event, pygame.event, pygame.event]           # 存储PCAct按键，依次是K_b, K_a, K_d
        self.winner = ""                                                     # 胜利者
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # 进入游戏Play界面
                    ...
                    self.PCActKey = [event.key, event.key, event.key]
    ...
    def draw_character(self, PC_pos, NPC_pos):
        # todo:set the draw order of the characters
        # 动态位置绘制
    ...
    def play_again_check(self):
        if self.playAgain is True:
            ...
            self.playerTurn = PlayerTurn.start
            self.PCActKey = [self.spaceKeyDown.key, self.spaceKeyDown.key, self.spaceKeyDown.key]
            self.cheat = [0, 0, 0]
            self.diceLocation = list()
            self.diceLocation.append(self.diceBoard[0])
            self.diceSteps = (0, 0, True)
...
while True:
    gameManager.event_deal()                                                              # 鼠标键盘事件处理
    ...
    if gameManager.gameTurn is GameTurn.initial:
        hero = PC("Naruto")                                                               # PC实例化
        enemy = NPC("Sasuke")                                                             # NPC实例化
