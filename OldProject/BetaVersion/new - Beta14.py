...
class GameManager:
    def __init__(self):
        ...
        self.font = pygame.font.Font(self.source + "simhei.ttf", 20)        # 游戏字体
        self.screen = pygame.display.set_mode((1200, 825))                  # 窗口初始化
        pygame.display.set_icon(self.__image_load("Dog.ico"))
        pygame.display.set_caption("大富翁")
        self.music = pygame.mixer.music.load(self.source + "BackgroundMusic.mp3")
    ...
    def post_space_key_down(self):
        pygame.event.post(self.spaceKeyDown)
    ...
    def __image_load(self, name):
        return pygame.image.load(self.source + name).convert_alpha()
    ...
    def turn_change_space(self):
        if self.playerTurn is PlayerTurn.start:
            ...
            pygame.time.delay(500)      # 用于产生一个较合理的回合切换效果，避免过快
            self.playerTurn = PlayerTurn.PCMove
