...
while order is False:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == pygame.BUTTON_LEFT:
            backgroundMusic.pause(event.pos)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # 千里马额外移动
            if hero.transportation == "千里马" and turnControl is Turn.PCAct:
                if event.key == K_a:        # 后退
    ...
    if (turnControl is Turn.PCMove or turnControl is Turn.NPCMove) and steps[2] is False:
        lenOfDiceLocation = len(diceLocation)
        clock.tick(10 * lenOfDiceLocation)
        for rand in shootDice.random_series:
            for i in range(lenOfDiceLocation):
                screen.blit(shootDice.image[rand], diceLocation[i])
            pygame.display.update()
        for i in range(lenOfDiceLocation):
            screen.blit(shootDice.final_image[i], diceLocation[i])
        pygame.display.update()
        pygame.time.delay(700)

    # 骰子掷过之后将骰子保留在原位置
    for i in range(len(diceLocation)):
        screen.blit(shootDice.final_image[i], diceLocation[i])
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == pygame.BUTTON_LEFT:
            ...
            # 音乐播放按钮图片绘制，全局化音乐按钮，重复绘制形成一种按按钮的动态效果
            screen.blit(backgroundMusic.image, (41 * 25, 27 * 25))
            pygame.display.update(backgroundMusic.buttonRect)
            pygame.time.delay(100)
            screen.blit(gameMap, (0, 0))
