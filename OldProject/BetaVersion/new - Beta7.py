...
musicOn = pygame.image.load("./source/MusicOn.png").convert_alpha()
musicOff = pygame.image.load("./source/MusicOff.png").convert_alpha()

# 音乐播放
pygame.mixer.init()
music = pygame.mixer.music.load("./source/Dexter Britain - Nothing To Fear.mp3")
musicPhoto = musicOn
pygame.mixer.music.set_volume(0.618)
pygame.mixer.music.play(-1)
currentStatus = "Unpause"
...
shootDice = ShootDice()
diceLocation = list()
diceLocation.append(shootDice.image_location[0])
steps = (0, 0, True)
...
for i in range(150):
    ...
    screen.blit(musicPhoto, (41 * 25, 27 * 25))
...
while order is False:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == pygame.BUTTON_LEFT:
            if event.pos[0] > (41 * 25 + 8) and event.pos[0] < (41 * 25 + 94) and \
               event.pos[1] > (27 * 25 + 4) and event.pos[1] < (27 * 25 + 68):
...
while True:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == K_SPACE:
                if turnControl is Turn.start:
                    turnControl = Turn.PCMove
                    steps = hero.move()
                    if steps[2] is False:
                        diceLocation = shootDice.set_dice(steps)
                    enemy.money += hero.incidents(lands)
                    hero.engine = 0
                elif turnControl is Turn.PCAct:
                    turnControl = Turn.NPCAct
                    break

            # PC购买地块、交换金钱、使用炸药
            if event.key == K_b and turnControl is Turn.PCAct:
    ...
    if turnControl is Turn.NPCMove:
        steps = enemy.move()
        if steps[2] is False:
            diceLocation = shootDice.set_dice(steps)
        hero.money += enemy.incidents(lands)
    elif turnControl is Turn.NPCAct:
    ...
    # 掷骰子动画播放
    if (turnControl is Turn.PCMove or turnControl is Turn.NPCMove) and steps[2] is False:
    ...
    # 回合切换
    if turnControl is Turn.PCMove:
        turnControl = Turn.NPCMove
    elif turnControl is Turn.NPCMove:
        turnControl = Turn.PCAct
    elif turnControl is Turn.NPCAct:
        turnControl = Turn.start
